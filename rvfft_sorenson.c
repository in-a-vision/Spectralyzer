...
/////////////////////////////////////////////////////////
// Take magnitude from each real/imag pair

void realfft( float *data, float *data2, long n ) {

	realfft_split_unshuffled( data, data2, n );

	float *d = data2;

	for( int i=0; i<n; i++ )
		d[i] = sqrt(d[i]*d[i] + d[i+1]*d[i+1]);
}
